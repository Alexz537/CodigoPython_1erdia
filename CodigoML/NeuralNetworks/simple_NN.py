...
num_labels = 10         # 10 etiquetas(neuronas finales) del 1 al 10
...
def sigmoid(z):
    g = 1 / (1 + np.exp(-z))#funcion red neuronal
    return g

#Calcular la precision
pred = predict(Theta_1, Theta_2, X)
print('Precision de los datos training :', np.mean(pred == y) * 100)#alimentar el programa 100 veces
...
for i in range(30):
    ...
    #display_data(X[i,].reshape(1, n))
    pred = predict(Theta_1, Theta_2, X[rp[i],].reshape(1, n))
    #pred = predict(Theta_1, Theta_2, X[i,].reshape(1, n))
    print ('Prediccion del Neural Network: {0} (digit {1})'.format(pred, pred % 10))
    plt.savefig('numero_'+str(pred)+'prednum_'+str(i)+'.png')
